...
{
    log(`Compiling ${chalk.green(this.name)} in ${(debug) ? chalk.yellow('debug') : chalk.green('release')} mode...`);
    ...
    .then(() =>
    {
        ...
        {
            log(`Manifest files will be generated in-place, in ${chalk.cyan(path.resolve(destination))}.`);
            return mkdirp(destination);
        }
        else
        {
            log(`Manifest files will be generated in ${chalk.cyan(path.resolve(destination))}.`);
            ...
            .then(() =>
            {
                ...
                log(`Copying bundle files from ${chalk.cyan(path.resolve(this.source))}`);
    ...
    .tap(() => log(`Build ${chalk.green(this.name)} compiled successfully.`));
};
...
Build.prototype.pack = function (stagingFolder, debug, packaging)
{
    ...
    // Compile the build to temp directory
    .then(() =>
    {
